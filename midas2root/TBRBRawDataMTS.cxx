...
TBRBRawDataMTS::TBRBRawDataMTS(int bklen, int bktype, const char* name, void *pdata):
    TGenericData(bklen, bktype, name, pdata)
{
  ...
  int nadcs = ((npackets - 1))/1;
  //  std::cout << "nadc " << nadcs << std::endl;
  ...
  for(int adc =0; adc < nadcs; adc++){ // loop over ADCs
    ...
    for(int ach = 0; ach < 4; ach++){
      ...
      //      std::cout << "N samples (chan="<<ch<<") " << meas.GetNSamples() << std::endl;
    }

    if(adc == 0 && 0){
      for(int i = 0; i < Samples[0].size(); i++){
	if(0)	std::cout << "index = " << i << " Samples (ch0,1,2,3): " << Samples[0][i] << " , " << Samples[1][i] << " , "
