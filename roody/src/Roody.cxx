...
// Roody.cxx
//
// $Id: Roody.cxx 245 2012-06-14 18:14:25Z olchansk $
//
...
void Roody::AddObjectToVec(PadObjectVec* padvec, TVirtualPad* pad, const ObjectPath* src, TObject* obj, const char* drawOpt)
{
  ...
  int padNum = pad->GetNumber();

  if (padNum == 0)
     padNum = 1;

  //printf("add object %p to pad %p number %d, draw opt \'%s\'\n", obj, pad, padNum, drawOpt);

  padvec->push_back(PadObject(padNum, xobj, src, drawOpt));
}
...
TCanvas* Roody::MakeNewCanvas(const char* title, int columns, int rows, int topx, int topy, int width, int height)
{
  ...
  //printf("MakeNewCanvas divide %d %d\n", columns, rows);
...
bool Roody::OpenXmlServer(char const *dest)
{
  if( fContents->FindChildByName(fTreeItemOnline, dest))
    {
#if 0
      std::string s("Error in loading file: \"");
  ...
  fOnlineFiles.push_back(dest);

  DataSourceXmlServer* d = new DataSourceXmlServer(dest);
