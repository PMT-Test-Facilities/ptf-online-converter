...
void SkimSingle::BeginRun(int transition,int run,int time){
    ...
    PeakHist1 = new TH1F("PeakHist1","PeakHist1",351,-50.5,300.5);
    PeakHist1->SetDirectory(0);
...
void SkimSingle::EndRun(int transition,int run,int time){
    ...
    ChargeHist->Fit("gaus","","",-50,50);
    ChargeHist->Write();
    BaselineHist->Write();
    TimeHist->Write();
    Double_t norm = 1;
    PeakHist->Scale(norm/PeakHist->Integral(), "width");
    PeakHist->Fit("gaus","R","",-10,10);
    PeakHist->Fit("gaus","R+","",10,20);
    PeakHist->Write();
    PeakHist->SaveAs("1112norm_mod");
    Charge_Time->Write();
    Peak_Time->Write();
...
bool SkimSingle::ProcessMidasEvent(TDataContainer& dataContainer){
	...
	if(v1730){
		...
		if(counter < 500){
                    Double_t Baseline = 0.;
                    for(int j=9520; j<9540; ++j) Baseline += (Double_t) samples[j]; //Thomas values 9520-9540
                    Baseline /= 20.;
                    Double_t Charge=0.;
                    for(int j=2480; j<2520; ++j) Charge += (Double_t) samples[j]; //Thomas values 9540-9580
            ...
            if(chan == 0){
                ...
                Double_t Baseline = 0.;
                for(int j=9500; j<9540; ++j) Baseline += (Double_t) samples[j];  //Thomas values 9500-9540
                Baseline /= 40.;
                Double_t Charge=0.;
                Double_t MaxPeak=100.;
                for(int j=2510; j<2550; ++j){ Charge += (Double_t) samples[j]; if(samples[j]-Baseline < MaxPeak) MaxPeak = samples[j]-Baseline;} //Thomas values 9540-9580
                if(counter < 500){
		...
		TF1 *gfit = new TF1("Gaussian","gaus",0,10);
		PeakHist1->Fill(-MaxPeak);
		PeakHist1->Fit(gfit,"RNQ");
		Double_t mean = gfit->GetParameter(1);
		//cout << "Mean: " <<-MaxPeak << endl;
		PeakHist->Fill(-MaxPeak-mean);
...
/*
            if(chan == 0){
                rootevent->SetTargetPMTSamples(samples);
                Double_t Baseline = 0.;
		Double_t std =0.; //Discover FWHM to subtract from MaxPeak to zero pedestal
                for(int j=9500; j<9540; ++j) Baseline += (Double_t) samples[j];  //Thomas values 9500-9540
                Baseline /= 40.;
		for(int j=9500; j<9540; ++j){ std += ((Double_t) samples[j]-Baseline)*((Double_t) samples[j]-Baseline);}
		std /= 40;
		std = sqrt(std);
                Double_t Charge=0.;
		Double_t MaxPeak=100.;
                ...
                BaselineHist->Fill(Baseline);
                TimeHist->Fill(PeakTime);
                if(-MaxPeak < 4.7*std){ PeakHist->Fill(-MaxPeak-2.35*std);} //this is not a perfect solution...I'm assuming anything outside of the FWHM of the pedestal is a pe which is a bit ofa stretch...to be refined later
                else PeakHist->Fill(-MaxPeak);
		Charge_Time->Fill(PeakTime,Charge);
                Peak_Time->Fill(PeakTime,Peak);
            }
		}		
