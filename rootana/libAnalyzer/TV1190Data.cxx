    ...
    TGenericData(bklen, bktype, name, pdata)
{
  ...
  // Make sure first word has right identifier
	...
	if(verbose_debugging)std::cerr << "First word has wrong identifier; first word = 0x" 
  ...
  if(found_trailer_word == -1){
    if(verbose_debugging)std::cerr << "Error in decoding V1190 data; didn't find trailer." << std::endl;
    if(verbose_debugging)std::cerr << "Bank dump: " << std::endl;
    for(int i = 0; i < GetSize(); i++){
      uint32_t word = GetData32()[i];
      if(verbose_debugging)std::cout << "0x"<<std::hex << word << std::dec << std::endl;
    }
    fCorruptedBank = true;
  }
  if(found_trailer_word != GetSize()-1){
    if(verbose_debugging)std::cerr << "Error; did not find trailer on last word of bank. trailer word = " << found_trailer_word
	      << " last word = " << GetSize()-1 << std::endl;
    fCorruptedBank = true;
  }
  if(fWordCountTotal != GetSize()){
    if(verbose_debugging)std::cerr << "Error in decoding V1190 data; word count in all trailers ("<<fWordCountTotal 
