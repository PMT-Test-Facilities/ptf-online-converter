...
#define nPoints_max 1000000
#define num_phidg_max 10000
#define num_v1730_max 20 // IMPOTANT: IF THIS IS EVER CHANGED, ALSO CHANGE THE HARDCODED VALUES FOR WAVEFORM BRANCH WIDTHS AS WELL (see: "v1730 data")
#define timeStart 20 // defines start of PMT Pulse timing window, currently at the 130th sample of 200, with a window size of 70 samples.
...
private:
  ...
  //Digitizer variables
    ...
    double V1730_wave0[nPoints_max][num_v1730_max],  V1730_wave1[nPoints_max][num_v1730_max],  V1730_wave2[nPoints_max][num_v1730_max], V1730_wave3[nPoints_max][num_v1730_max];
    double V1730_wave4[nPoints_max][num_v1730_max],  V1730_wave5[nPoints_max][num_v1730_max],  V1730_wave6[nPoints_max][num_v1730_max], V1730_wave7[nPoints_max][num_v1730_max];
    double V1730_wave8[nPoints_max][num_v1730_max],  V1730_wave9[nPoints_max][num_v1730_max],  V1730_wave10[nPoints_max][num_v1730_max], V1730_wave11[nPoints_max][num_v1730_max];
    double V1730_wave12[nPoints_max][num_v1730_max],  V1730_wave13[nPoints_max][num_v1730_max],  V1730_wave14[nPoints_max][num_v1730_max], V1730_wave15[nPoints_max][num_v1730_max];
...
public:
  ...
  void BeginRun(int transition,int run,int time){
    ...
    tree->Branch("V1730_wave0",&V1730_wave0,"V1730_wave0[num_points][20]/Double_t"); //think of eqn* // SIZE OF COLUMN MUST MATCH num_v1730_max OR ELSE BANDING ISSUES WILL OCCUR
    tree->Branch("V1730_wave1",&V1730_wave1,"V1730_wave1[num_points][20]/Double_t"); //think of eqn*
    tree->Branch("V1730_wave2",&V1730_wave2,"V1730_wave2[num_points][20]/Double_t"); //think of eqn*
    tree->Branch("V1730_wave3",&V1730_wave3,"V1730_wave3[num_points][20]/Double_t"); 
    tree->Branch("V1730_wave4",&V1730_wave4,"V1730_wave4[num_points][20]/Double_t");
    tree->Branch("V1730_wave5",&V1730_wave5,"V1730_wave5[num_points][20]/Double_t");
    tree->Branch("V1730_wave6",&V1730_wave6,"V1730_wave6[num_points][20]/Double_t");
    tree->Branch("V1730_wave7",&V1730_wave7,"V1730_wave7[num_points][20]/Double_t");
    tree->Branch("V1730_wave8",&V1730_wave8,"V1730_wave8[num_points][20]/Double_t");
    tree->Branch("V1730_wave9",&V1730_wave9,"V1730_wave9[num_points][20]/Double_t");
    tree->Branch("V1730_wave10",&V1730_wave10,"V1730_wave10[num_points][20]/Double_t");
    tree->Branch("V1730_wave11",&V1730_wave11,"V1730_wave11[num_points][20]/Double_t");
    tree->Branch("V1730_wave12",&V1730_wave12,"V1730_wave12[num_points][20]/Double_t");
    tree->Branch("V1730_wave13",&V1730_wave13,"V1730_wave13[num_points][20]/Double_t");
    tree->Branch("V1730_wave14",&V1730_wave14,"V1730_wave14[num_points][20]/Double_t");
    tree->Branch("V1730_wave15",&V1730_wave15,"V1730_wave15[num_points][20]/Double_t");
...
//attempt at adding digitizer data
	...
	start_val_stat = 90;
	  ...
	  for(int i = 0; i < measurements.size(); i++){
	      ...
	      // 9.Nov.2017
	      // these channels are not in use, so comment them out to reduce file size
	      ...
	      // 	  for (int j = 0; j<samples.size(); ++j) SampleWave2->Fill(j, (Double_t)samples[j]);
	      // 	  SampleWave2->Write(Form("Chan_2_Wave_%d", counter));
	      // 	  SampleWave2->Reset();
	      // 	  ADC2_dig[num_points_dig1-1] = (double) -(Charge - Baseline*window_width);
	      // }
	      // if(chan == 3) {
	      // 	  Double_t Baseline = 0;
	      // 	  for(int j = 185; j < 245; ++j) Baseline += (Double_t) samples[j];
	      ...
	      // 	  for (int j = 0; j<samples.size(); ++j) SampleWave3->Fill(j, (Double_t)samples[j]);
	      // 	  SampleWave3->Write(Form("Chan_3_Wave_%d", counter));
	      // 	  SampleWave3->Reset();
	      // 	  ADC3_dig[num_points_dig1-1] = (double) -(Charge - Baseline*window_width);
	      // }

	      // 5.Dec.2017 testing G1 receiver PMT
	     //  if (chan == 4) {
	     //  	  Double_t Baseline = 0;
	     //  	  for (int j = 185; j < 245; ++j) Baseline += (Double_t) samples[j];
	     //  	  Baseline /= 60;
	     ...
	     //  	  for (int j = 0; j<samples.size(); ++j) SampleWave4->Fill(j, (Double_t)samples[j]);
	     //  	  SampleWave4->Write(Form("Chan_4_Wave_%d", counter));
	     //  	  SampleWave4->Reset();
	     //  	  ADC4_dig[num_points_dig1-1] = (double) -(Charge - Baseline*window_width);
	     //  }
...
switch (chan){
	...
	// save event counter, to compare to TDC event counter
	if(tdcbank && tdcbank->IsBankCorrupted()) nbadTDCbanks++;

	if(tdcbank && !tdcbank->IsBankCorrupted()){
	...
	return true;
	  ...
	  if(bank || 1){
		  ...
		  for(int ib = timeStart; ib < measurements[i].GetNSamples() && ib  < num_v1730_max + timeStart ; ib++){
		      ...
		      if(chan == 1) V1730_wave1[num_points-1][ib-timeStart] = measurements[i].GetSample(ib); // -130 is the timeStart offset, meaning the reference signal window starts at the start of the digitizer readout
		      if(chan == 2) V1730_wave2[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 3) V1730_wave3[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 4) V1730_wave4[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 5) V1730_wave5[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 6) V1730_wave6[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 7) V1730_wave7[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 8) V1730_wave8[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 9) V1730_wave9[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 10) V1730_wave10[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 11) V1730_wave11[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 12) V1730_wave12[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 13) V1730_wave13[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 14) V1730_wave14[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
		      if(chan == 15) V1730_wave15[num_points-1][ib-timeStart] = measurements[i].GetSample(ib);
      ...
      if(bank_ph0){
	num_phidg0_points++;
	if(num_phidg0_points > 10) std::cout << "Lots of phidget0 points!: " << num_phidg0_points << std::endl;
	...
	return true;
      ...
      if(bank_ph1){
	num_phidg1_points++;
	if(num_phidg1_points > 10) std::cout << "Lots of phidget points!: " << num_phidg1_points << std::endl;
	...
	std::cout << "Gantry bank: " << counter_gant << " " 
		  << time << " " << ((double*)bank_cyc->GetData64())[11] 
		  << std::endl;
