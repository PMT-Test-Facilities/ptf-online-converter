...
private:
  ...
  //Digitizer variables
  double ADC0_dig[nPoints_max], ADC1_dig[nPoints_max], Start_time0[nPoints_max], Window_width0[nPoints_max], Start_time1[nPoints_max], Window_width1[nPoints_max];
...
//attempt at adding digitizer data
      ...
      if(v1730){
        ...
        for(int i=0; i < measurements.size(); i=i+2){
...
//std::cout << "this is  " << tem << "  " << j << "  " << AbsDeriv << std::endl;
		Start_vec[tem] = Start;
...
std::cout << "absderiv  " << AbsDeriv << "  " << j << std::endl;  
            inpulse = true;
              MaxV= samples[j];
              Start = j-2;
              MaxVT = j;
	  ...
	  for(int ic = 0; ic < measurements[i+1].GetNSamples(); ic++){
            samples.push_back(measurements[i+1].GetSample(ic)-offset);
	  }
  	  double Deriv_1=0;
          double OldDeriv_1=0;
          double MaxV_1;
	  ...
	  std::vector<Int_t> BaseSamples_1;
          for(double m = 1; m<samples.size()-1; ++m){
            OldDeriv_1 = Deriv_1;
            Deriv_1 = samples[m+1] - samples[m-1];
	      ...
	      if((AbsDeriv_1 < SmallDeriv && (samples[m]-Baseline_1) > -5)|| m>(samples.size()-2)){
		for(int s=0; s<tem; s++){
		  if(Start_vec[s]+5 > Start_1 && Start_vec[s]-5 < Start_1){
	//	    num_points_dig0++;
//std::cout << "Num points " << std::endl;
		    ...
		    //Window_width1[num_points_dig1-1] = (double) m-Start_1+1;	    		  
	    ...
	    else if(AbsDeriv_1 > DerivThreshold){
              inpulse_1 = true;
              MaxV_1= samples[m];
              Start_1 = m-2;
              MaxVT_1 = m;
              ...
              MaxD_1 = Deriv_1;
              MaxDT_1 = m;
              Charge_1 = samples[m] + samples[m-1] + samples[m-2];
              Baseline_1 = 0;
              for(int n=0; n<BaseSamples_1.size(); ++n){ Baseline_1 += ((Double_t)BaseSamples_1[n])/((Double_t)BaseSamples_1.size()); }
            }
            else if(AbsDeriv_1 < SmallDeriv){
              BaseSamples_1.push_back(samples[m]);
