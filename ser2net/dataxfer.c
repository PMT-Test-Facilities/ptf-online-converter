...
setup_tcp_port(port_info_t *port)
{
    ...
    if (strncmp(port->devname,"usb",3) == 0)
      {
	extern char* usbFindSerial(int debug,const char* usbpath);
	char* s = usbFindSerial(1,port->devname);
	if (s == NULL) {
	  close(port->tcpfd);
	  syslog(LOG_ERR, "Could not find USB device for %s",
		 port->devname);
	  return -1;
	}

	syslog(LOG_ERR, "USB device %s is %s", port->devname, s);
	port->devfd = open(s, options);
      }
    else
      {
