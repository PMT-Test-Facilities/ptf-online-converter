  ...
  for (int i=0; i<wcount; i++)
    {
      ...
      switch (w & 0xF0000000)
        ...
        case 0x80000000: // event header
	     ...
	     //printf("*** Unit %d, trigger %d: Event header for trigger %d without an event trailer for previous trigger %d, group %d\n", unit, trigNo, trigNo, c->headerTrigNo, c->group);
	     //fBadDataCount++;
		...
		if (!timestampBases[unit])
	      ...
	      if (ts48 == ts[unit][cgroup])
		{
		  printf("*** Unit %d, trigger %d: Group %d has invalid timestamp %d, should be more than %d\n", unit, headerTrigNo, group, (int)ts48, (int)ts[unit][cgroup]);
		  fBadDataCount++;
		  ts48 = ts[unit][cgroup] + (int)(fCoinc*fFreq[unit])+1;
		}
	      
	      ts[unit][cgroup] = ts48;

	      double timestamp = (ts48 - timestampBases[unit])/fFreq[unit];
	     ...
	     if (!m || (chan<0) || (chan>=VF48_MAX_CHANNELS))
                 {
                    ...
                    fBadDataCount++;
                    printf("*** Unit %d, trigger %d: Unexpected data at %5d: 0x%08x (no module %p or bad channel number %d)\n", unit, headerTrigNo, i, w, m, chan);
  ...
  for (int i=0; i<size32; i++)
    {
      ...
      if ((w & 0xF0000000) == 0xF0000000) // group marker
        {
          int group = (w & 0x00000007);
	  ...
	  //wbuf[unit][grp][wptr[unit][grp]++] = w;
      ...
      else if ((w & 0xF0000000) == 0x80000000) // event header
        {
          if (countSkipped[unit])
            printf("*** Unit %d, Skipped %d words while waiting for event header\n", unit, countSkipped[unit]);

	  if (fDisasmStream) {
	    printf("UnpackVF48A: Header  0x%08x, unit %d, group %d\n", w, unit, grp);
	  }
	  ...
	  if (fDisasmStream) {
	    printf("UnpackVF48A: Trailer 0x%08x, unit %d, group %d\n", w, unit, grp);
	  }
