...
static MVME_INTERFACE *gVme;
static int gBase = 0;
static int debug = 0;

void vme_open(uint32_t addr)
{
   int status = mvme_open(&gVme, 0);
   assert(status == MVME_SUCCESS);
...
void vme_write(uint16_t v)
{
   mvme_write_value(gVme, gBase, v);
}

uint16_t vme_read()
{
   return mvme_read_value(gVme, gBase);
}
...
int bb_verify( int *types )
{
   ...
   if ((v1 & 0xFF) != 0xCD)
      {
         fprintf( stderr, "Error: Verifying hardware: ByteBlaster not found, VME 0x%08x read 0x%08x should be 0xXXXXXXcd!\n", gBase, v1);
         return CB_BB_VERIFY_BYTEBLASTER_NOT_FOUND;
      }

   fprintf( stdout, "Info: Found VME 0xABCD FlashProgrammer interface.\n" );
   *types = BBMV;
   return CB_OK;
...
int bb_lptread( int port, int *data )
{
   ...
   *data = (temp>>8) & 0xff;
   ...
   if (debug)
      printf("bb_lptread  port %d, data 0x%02x, vme read 0x%08x\n", port, *data, temp);
...
int bb_lptwrite( int port, int data, int nbuffering )
{
   ...
   if (debug)
      printf("bb_lptwrite port %d, data 0x%02x, nbuffering %d, vme write 0x%08x\n", port, data, nbuffering, w);
...
/********************************************************************************/
int fs_open( char argv[], char* mode, FILE** file_id )
{
	FILE* fid;
	fid = fopen( argv, mode );
	if ( fid == NULL )
...
int as_silicon_id(int file_size, int epcsDensity)
{
	...
	int silicon_ID = 0;
		...
		{
                   fprintf( stdout, "\nError: Unsupported Device 0x%x", silicon_ID);
...
/* Version Number */
const char VERSION[] = "1.2-VME-0xABCD-KO2 (POF, JIC support)";
...
void help()
{
   fprintf( stdout, " \n Command\t\t\t\t\t\tDescription\n");	
   fprintf( stdout, " ============\t\t\t\t\t\t===============\n");	
   fprintf( stdout, " srunner -program -<EPCS density in Mb> <file.rpd> <vme address>\t=> Program EPCS\n");	
   fprintf( stdout, " srunner -read -<EPCS density in Mb> <file.rpd> <vme address>\t\t=> Read EPCS data to file\n");
   fprintf( stdout, " srunner -verify -<EPCS density in Mb> <file.rpd> <vme address>\t=> Verify EPCS data with file\n");
   fprintf( stdout, " srunner -id -<EPCS density in Mb> <file.rpd> <vme address>\t=> Identify EPCS device\n");
   fprintf( stdout, "\nExample of command -> srunner -program -64 Mydesign.rpd 0xa00020\n");
   fprintf( stdout, "\nExample of command -> srunner -id -64 /dev/null 0xa00020\n");
}
...
int main( int argc, char** argv)
{
	...
	else
	{
		epcsDensity = atoi(&argv[2][1]);
                ...
                printf("Using VME address 0x%08x\n", addr);
		...
		status = as_id();		//Execute identify function
		}
